...
public:
	...
	addr_t pca_nn_start_addr = 0x75000000;
	addr_t pca_nn_size = 0x01000000;
	addr_t pca_nn_end_addr = pca_nn_start_addr + pca_nn_size - 1;
	...
	void parse(int argc, char **argv) override {
		...
		assert((mem_end_addr < clint_start_addr || mem_start_addr > pca_nn_end_addr) &&
...
int sc_main(int argc, char **argv) {
	...
	bus.ports[12] = new PortMapping(opt.pca_nn_start_addr, opt.pca_nn_end_addr);
	...
	bus.isocks[12].bind(pca_nn.tsock);
