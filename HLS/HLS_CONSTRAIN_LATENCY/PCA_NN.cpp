...
void PCA_NN::do_filter()
{
	...
	while (true)
	{
		...
		for (sc_uint<12> i = 0; i < 1024; i++)
		{
			...
			input_data -= mean[i];
			//HLS_CONSTRAIN_LATENCY(0, 2, "algorithm_latency01");
			pca_result[0] += input_data * coeff[i];
			wait();
			//HLS_CONSTRAIN_LATENCY(0, 2, "algorithm_latency02");
		...
		{
			HLS_CONSTRAIN_LATENCY(0, 4, "algorithm_latency03");
		...
		for (sc_uint<4> i = 0; i < 3; i++)
		{
			...
			{
				HLS_CONSTRAIN_LATENCY(0, 2, "algorithm_latency04");
