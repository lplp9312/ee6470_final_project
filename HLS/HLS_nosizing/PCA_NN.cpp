...
void PCA_NN::do_filter()
{
	...
	sc_fixed<32, 8> input_data;
	sc_fixed<32, 8> pca_result[2];
	sc_fixed<32, 8> hidden_o[4];
	sc_fixed<32, 8> output_o[3];
	sc_uint<32> prediction;
	...
	while (true)
	{
		...
		for (sc_uint<32> i = 0; i < 3; i++)
		{
			output_o[i] = 0;
			for (sc_uint<32> j = 0; j < 4; j++)
			{
